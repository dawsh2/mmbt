...
class PositionManager:
    ...
    def on_signal(self, event):
        ...
        # Case 1: Event object with SignalEvent in data
        if isinstance(event, Event) and hasattr(event, 'data'):
            signal = event.data
        # Case 2: SignalEvent directly
        elif hasattr(event, 'get_signal_value') and callable(getattr(event, 'get_signal_value')):
            signal = event
        else:
            logger.error(f"Received invalid signal object: {type(event)}")
            return []
        ...
        if self.event_bus and actions:
            for action in actions:
                # Make sure action_type is always present and correct
    ...
    def _process_signal(self, signal):
        ...
        """
        # Type validation - ensure we have proper signal methods
        if not (hasattr(signal, 'get_signal_value') and callable(getattr(signal, 'get_signal_value'))):
            logger.error(f"Invalid signal object: {type(signal)}")
            return []
        
        actions = []
        
        # Get signal data using getters
        try:
            symbol = signal.get_symbol() if hasattr(signal, 'get_symbol') else "default"
            direction = signal.get_signal_value()
            price = signal.get_price() if hasattr(signal, 'get_price') else None
            timestamp = getattr(signal, 'timestamp', datetime.datetime.now())
            
            # Add validation for price
            if price is None or price <= 0:
                logger.warning(f"Invalid price in signal: {price}")
                return []
        ...
        for position in positions_to_exit:
            # Create exit action with explicit action_type
            exit_action = {
                'action_type': 'exit',
        ...
        if should_enter:
            ...
            if position_size != 0:
                # Create entry action with explicit action_type
                entry_action = {
                    'action_type': 'entry',
                    'symbol': symbol,
                    ...
                    'stop_loss': None,
                    'take_profit': None,
                    'strategy_id': getattr(signal, 'rule_id', None) if hasattr(signal, 'rule_id') else None,
                    'timestamp': timestamp
                }
    ...
    def _calculate_position_size(self, signal):
        ...
        try:
            ...
            logger.error(f"Error getting signal attributes: {e}")
            return 0
    ...
    def _execute_entry_action(self, action, current_time):
        """
        ...
        try:
            ...
            # Check for existing positions with the same symbol and direction
            # This helps avoid duplicate entries
            has_matching_position = False
            if hasattr(self.portfolio, 'positions_by_symbol') and symbol in self.portfolio.positions_by_symbol:
                for pos in self.portfolio.positions_by_symbol[symbol]:
                    if getattr(pos, 'direction', 0) == direction:
                        # Already have a position in this direction
                        has_matching_position = True
                        logger.info(f"Already have a {direction > 0 and 'LONG' or 'SHORT'} position for {symbol}, considering adding to it")
                        # Could implement position adding logic here if desired
                        break

            # If we already have a matching position, we might:
            # 1. Skip creating another one
            # 2. Add to the existing one
            # 3. Create a new one anyway
            # Let's implement option 1 as it's simplest
            if has_matching_position and False:  # Set to True to enable skipping
                logger.info(f"Skipping duplicate position creation for {symbol}")
                return {
                    'action_type': 'entry',
                    'success': True,
                    'symbol': symbol,
                    ...
                    'price': price,
                    'time': current_time,
                    'info': 'Position already exists'
                }

            # Continue with normal position creation
    ...
    def _execute_exit_action(self, action, current_time):
        """
        Execute an exit action with position existence check.
        """
        try:
            # Extract position_id and price from the action
            if isinstance(action, dict):
            ...
            if not all([position_id, price]):
                logger.error(f"Missing required fields for exit action")
                return {'action_type': 'exit', 'success': False, 'error': 'Missing required fields'}

            # Check if position exists before attempting to close
            # This prevents the "Position not found" error when portfolio has already closed it
            position_exists = False
            if hasattr(self.portfolio, 'positions'):
                position_exists = position_id in self.portfolio.positions
            ...
            if not position_exists:
                logger.info(f"Position {position_id} already closed or doesn't exist, skipping")
                return {
                    'action_type': 'exit',
                    'success': True,  # Mark as success to avoid error state
                    'position_id': position_id,
                    'symbol': '',  # Can't determine symbol without position
                    'price': price,
                    'pnl': 0,  # Can't determine PnL
                    'time': current_time,
                    'info': 'Position already closed'
                }

            # Rest of the original function...
            # Handle exit_type
            if isinstance(exit_type, str) and hasattr(ExitType, exit_type.upper()):
                exit_type = getattr(ExitType, exit_type.upper())
