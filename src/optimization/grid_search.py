    ...
    def optimize(self, configs, data_handler, metric='return', verbose=True):
        ...
        for i, (component_class, param_sets) in enumerate(expanded_configs):
            ...
            for params in param_sets:
                ...
                # Make a deep copy of the parameters to avoid reference issues
                import copy
                ...
                # Store the result with the parameter set as key
                param_tuple = tuple(sorted(params.items()))
                component_performances[param_tuple] = score
        ...
        for idx, data in top_components:
            ...
            component_class = data[2]

            if verbose:
                print(f"Creating optimized {component_class.__name__} with params: {best_params}")

            # Create the component with best parameters
            self.best_components[idx] = self.component_factory.create(component_class, best_params)
