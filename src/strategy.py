...
from rules import TradingRules, RuleSystem  # Assuming rules.py is in the same directory
...
class TopNStrategy(Strategy):
    ...
    def generate_signals(self, OHLC, filter_regime=False):
        """Generate trading signals using the top N rules."""
        final_signal = self.rules.generate_signals(OHLC)
...
class WeightedStrategy(Strategy):
    ...
    def _optimize_weights(self, OHLC, params, ga_module):
        ...
        # Assuming RuleSystem.generate_signals returns a DataFrame with combined signals
        # You might need to adjust this based on the actual output
        if isinstance(signals_df, pd.Series):
            X = signals_df.values.reshape(-1, 1) # Or handle differently if it's a single series
        elif isinstance(signals_df, pd.DataFrame):
            X = signals_df.values
        else:
            raise ValueError("Unexpected output from rules.generate_signals for GA")
    ...
    def generate_signals(self, OHLC, filter_regime=False):
        ...
        if filter_regime:
            # Assuming _apply_regime_filter now takes the OHLC DataFrame
            final_signal = self._apply_regime_filter(final_signal, OHLC)
...
# from rules import TradingRules, RuleSystem
...
#         """Initialize the strategy with configuration."""
#         self.config = config
#         self.rules = RuleSystem() #TradingRules()
#         self.signals = None
#         self.weights = None
