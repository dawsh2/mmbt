...
try:
    ...
    components=[sma_rule],
        weights=[1.0],
        buy_threshold=0.001,  # Much lower threshold
        sell_threshold=-0.001,  # Much lower threshold
...
try:
    ...
    'fast_window': 10,
        'slow_window': 30,
        'smooth_signals': False  # Add this line
    })  
...
# Add this after running the backtest but before the debug section
...
df['Crossover'] = np.where(df['SMA_10'] > df['SMA_30'], 1, -1)
df['Signal'] = df['Crossover'].diff()
...
logger.info(f"Found {len(crossovers)} SMA crossover points in the data")
if not crossovers.empty:
    logger.info(f"Crossover points: {crossovers[['timestamp', 'Close', 'SMA_10', 'SMA_30', 'Signal']].head().to_dict()}")
else:
    logger.info("No SMA crossovers found in the data - this might explain the lack of trades")
...
# Specifically check for signals
if 'signals' in results:
    logger.info(f"Number of signals: {len(results['signals'])}")
    if results['signals']:
        logger.info(f"Sample signals: {results['signals'][:3]}")
else:
    logger.info("No 'signals' key in results")    
...
logger.info("\nPossible solutions to try:")
logger.info("1. Lower the strategy thresholds (e.g., buy_threshold=0.001, sell_threshold=-0.001)")
logger.info("2. Try different SMA periods (e.g., fast_window=5, slow_window=15)")
logger.info("3. Use a different rule type that might generate more signals")
logger.info("4. Check if the execution engine is properly converting signals to orders")
logger.info("5. Verify that the backtester is correctly passing events between components")
