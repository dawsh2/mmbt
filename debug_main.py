#!/usr/bin/env python3
# Updated main.py with position manager signal debugging
...
from src.events.event_handlers import LoggingHandler, FillHandler
...
def main():
    ...
    try:
        ...
        # Add debug wrapper for strategy output
        ...
        # Add signal to position manager debug wrapper
        def signal_to_position_manager(event):
            """Debug wrapper to ensure position manager receives the right data structure"""
            print("\n==== SIGNAL EVENT DEBUG ====")
            ...
            if hasattr(event, 'data'):
                ...
                # If it's a Signal object, convert to dictionary
                if hasattr(signal_data, 'signal_type') and not isinstance(signal_data, dict):
                    print("Converting Signal object to dictionary...")
                    signal_dict = {
                        'symbol': getattr(signal_data, 'symbol', 'SYNTHETIC'),
                        'signal_type': signal_data.signal_type.value if hasattr(signal_data.signal_type, 'value') else signal_data.signal_type,
                        'direction': signal_data.signal_type.value if hasattr(signal_data.signal_type, 'value') else signal_data.signal_type,
                        'price': getattr(signal_data, 'price', None),
                        'confidence': getattr(signal_data, 'confidence', 1.0),
                        'timestamp': getattr(signal_data, 'timestamp', datetime.datetime.now())
                    }
                    print(f"Converted signal dict: {signal_dict}")
                    signal_data = signal_dict
                
                print(f"Calling position_manager.on_signal with: {signal_data}")
                try:
                    result = position_manager.on_signal(signal_data)
                    print(f"Position manager result: {result}")
                    
                    # If we got actions, let's emit orders directly
                    if result and isinstance(result, list) and len(result) > 0:
                        print(f"Position manager generated {len(result)} actions")
                        for action in result:
                            if action.get('action') == 'entry':
                                # Convert action to order and emit
                                print(f"Converting action to order: {action}")
                                order = {
                                    'order_id': str(uuid.uuid4()),
                                    'symbol': action.get('symbol'),
                                    'direction': action.get('direction'),
                                    'quantity': action.get('size'),
                                    'price': action.get('price'),
                                    'order_type': 'MARKET',
                                    'timestamp': datetime.datetime.now()
                                }
                                print(f"Emitting ORDER event: {order}")
                                event_bus.emit(Event(EventType.ORDER, order))
                            else:
                                print(f"Action type not handled: {action.get('action')}")
                    
                    return result
                except Exception as e:
                    print(f"ERROR in position_manager.on_signal: {e}")
                    import traceback
                    traceback.print_exc()
                    return None
            else:
                print("Event has no data attribute!")
                return None



        def debug_order_execution(event):
            """Debug wrapper to ensure execution engine processes orders correctly"""
            print("\n==== ORDER EVENT DEBUG ====")
            print(f"Event type: {event.event_type}")
            ...
            if hasattr(event, 'data'):
                ...
                try:
                    # Call the execution engine with the order data
                    print(f"Calling execution_engine.on_order with: {order_data}")
                    result = execution_engine.on_order(event)
                    print(f"Execution engine result: {result}")

                    # Manually create a fill event as a fallback
                    # In debug_order_execution function:
                    if not any(e.event_type == EventType.FILL for e in event_bus.history[-5:]):
                        print("No FILL event detected, manually creating one...")
                        fill_data = {
                            'order_id': order_data['order_id'],
                            'symbol': order_data['symbol'],
                            'direction': order_data['direction'],
                            'quantity': order_data['quantity'],
                            'fill_price': order_data['price'] * (1 + 0.0005 * order_data['direction']),  # Apply slippage
                            'timestamp': datetime.datetime.now(),
                            'commission': order_data['price'] * order_data['quantity'] * 0.001  # 10 bps commission
                        }
                        print(f"Emitting manual FILL event with data: {fill_data}")
                        fill_event = Event(EventType.FILL, fill_data)
                        event_bus.emit(fill_event)

                        # Directly call position_manager.on_fill to debug
                        print("Directly calling position_manager.on_fill")
                        position_manager.on_fill(fill_event)
                    # if not any(e.event_type == EventType.FILL for e in event_bus.history[-5:]):
                    #     print("No FILL event detected, manually creating one...")
                    #     fill_data = {
                    #         'order_id': order_data['order_id'],
                    ...
                    #         'direction': order_data['direction'],
                    #         'quantity': order_data['quantity'],
                    #         'fill_price': order_data['price'] * (1 + 0.0005 if order_data['direction'] == 1 else 1 - 0.0005),  # Simple slippage
                    #         'timestamp': datetime.datetime.now(),
                    #         'commission': order_data['price'] * order_data['quantity'] * 0.001  # 10 bps commission
                    #     }
                    #     print(f"Emitting manual FILL event: {fill_data}")
                    #     event_bus.emit(Event(EventType.FILL, fill_data))

                    return result
                except Exception as e:
                    print(f"ERROR in execution_engine.on_order: {e}")
                    import traceback
                    traceback.print_exc()
                    return None
            else:
        ...
        # Clear any existing registrations for these event types
        event_bus._handlers = {event_type: [] for event_type in EventType}  # Only if you have access to this

        # Then register your handlers
        event_bus.register(EventType.BAR, debug_strategy_output)
        event_bus.register(EventType.SIGNAL, signal_to_position_manager) 
        event_bus.register(EventType.ORDER, debug_order_execution)
        event_bus.register(EventType.FILL, position_manager.on_fill)
        event_bus.register(EventType.PARTIAL_FILL, position_manager.on_fill)
            
        # OR Option 2: Use the FillHandler if you prefer
        ...
        for bar in data_handler.iter_train():
            ...
            # Add a check for positions
            if bar_count % 20 == 0 or len(portfolio.positions) > 0:
                print(f"\nBar {bar_count} @ {current_timestamp}")
                print(f"Current price: ${current_price:.2f}")
                print(f"Portfolio positions: {len(portfolio.positions)}")
                for pos_id, pos in portfolio.positions.items():
                    print(f"  Position {pos_id}: {pos.symbol} {pos.quantity} @ {pos.entry_price}")
